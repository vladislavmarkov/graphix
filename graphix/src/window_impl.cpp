...
window::make_shared(
    ...
    uint32_t           height)
{
    return std::shared_ptr<window>{std::make_shared<gfx_impl::window_impl>(
...
window::make_unique(
    ...
    uint32_t           height)
{
    return std::unique_ptr<window>{std::make_unique<gfx_impl::window_impl>(
        static_cast<gfx_impl::application_impl&>(app),
        caption,
        pos_x,
        pos_y,
...
window_impl::~window_impl() { _app.detach_window(this); }
uint32_t
window_impl::get_id() const
{
