...
std::shared_ptr<application> application::make_shared(){
    return std::shared_ptr<application>{
        std::make_shared<gfx_impl::application_impl>()
    };
}

std::unique_ptr<application> application::make_unique(){
...
void application_impl::manage_pending_windows(){
    SDL_PumpEvents();
    SDL_Event event;
    while(
        SDL_PeepEvents(
...
void do_attach_window(
    ...
    if (wnd_rec != wnd_ptrs.end()){
        throw gfx::error(
            std::string("can't attach window with id ") +
...
void do_detach_window(
    ...
    if (wnd_rec == wnd_ptrs.end()){
        throw gfx::error(
            std::string("there is no window with id ") +
            std::to_string(id)
        );
    }
...
void application_impl::handle_attach_detach_event(const SDL_Event *event){
    assert(event);
...
void application_impl::register_messages(){
...
bool application_impl::handle_event(const SDL_Event *event){
    assert(event);
...
application_impl::application_impl():
    _attach_window_msg_type(0),
    _detach_window_msg_type(0)
{
...
int application_impl::run(){
    ...
    while (!done){
        if (SDL_WaitEvent(&e) != 0){ done = handle_event(&e); }
...
void application_impl::attach_window(window_impl *wnd){
    assert(wnd);
    SDL_Event event;
...
void application_impl::detach_window(window_impl *wnd){
    assert(wnd);
    SDL_Event event;
    SDL_memset(&event, 0, sizeof(event));
